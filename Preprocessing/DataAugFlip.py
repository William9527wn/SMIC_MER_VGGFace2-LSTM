...
#Data preprocessing version 3.0
#1. Random steps between frames within a slide window;
#2. Variable sele_frame(slide window size)
#3. Variabl-size of slide window(if image length smaller than the seleframe, window size = image length);
#4. Mirror flipping
    ...
    for group in tra_tes_list:    
        for folder, exp in zip(group[0], group[1]):
            ...
            if img_len <= seleframe: #for frames less than seleframe
                sequ_list.append(img_list)#append the sequence directly
                exp_list.append(exp)
